...
const displayFolderStructure = async()=>{
    ...
    for(const fileOrFolder in currentFolderStructure){
        ...
        sizeCell.textContent = currentFolderStructure[fileOrFolder]["fileSize"] ? formatFileSize(currentFolderStructure[fileOrFolder]["fileSize"]) : 'N/A';
...
const updateInfoPanel = ()=>{
    storagePercentCircle(userDataFromDB.Total_storage, userDataFromDB.Used_storage);
    document.getElementById("total-storage-value").textContent = userDataFromDB.Total_storage;
    document.getElementById("used-storage-value").textContent = userDataFromDB.Used_storage;
    document.getElementById("left-storage-value").textContent = userDataFromDB.Total_storage - userDataFromDB.Used_storage;

    updateFilesCount(userDataFromDB.image_count, userDataFromDB.videos_count, userDataFromDB.document_count, userDataFromDB.other_count);
};
...
const viewFile = async (fileKey)=>{
    ...
    const response = await fetch(`/files/presignedUrl?fileKey=${encodeURIComponent(fileKey)}`,{
            method: 'GET',
        });
    if (!response.ok) {
...
const deleteFile = async (event, fileName, fileKey)=>{ //to-do : count & size
    if(confirm("Are you sure want to delete this file "+fileName)){
        startLoading("Deleting File...");

        const response = await fetch(`/files/delete?fileKey=${encodeURIComponent(fileKey)}`,{
            method : 'DELETE',
        });
        if(response.ok){
...
const deleteFolder = async (event, folderName)=>{ //to-do : count & size
    if(confirm("Are you sure want to delete this file "+folderName)){
        ...
        const response = await fetch(`/files/delete?fileKey=${encodeURIComponent(folderKey)}&folder=true`,{
            method : 'DELETE',
        });
        if(response.ok){
...
const signOut = async()=>{
    ...
    try{
        ...
        endLoading();
        if(response.ok){
            const logoutUrl = `https://filebox-user-authentication.auth.eu-north-1.amazoncognito.com/logout?client_id=${encodeURIComponent(CLIENT_ID)}&logout_uri=${encodeURIComponent('http://localhost:5000/homepage')}`;
...
const updateFilesCount = async(imagesCount,videosCount,documentsCount,othersCount)=>{
